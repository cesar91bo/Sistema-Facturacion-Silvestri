...
{
    public partial class frmFibraOptica : Form
    {
        public bool Seleccion;
        List<VistaClientes> clis;
        public Int32 NroCliente;
        public frmFibraOptica()
        {
            InitializeComponent();
        }

        private void frmFibraOptica_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void CalculaTotalCliente()
        {
            ...
            txtTotalActivo.Text = cliNegocio.CalculaTotalFibra().ToString();
        }
        ...
        private void LLenarListaCliente(bool _siload)
        {
            try
            {
                ...
                {
                    var cliNegocio = new ClienteNegocio();
                    clis = cliNegocio.ObtenerClientes();
                }
                else { clis = BuscarCli(); }
        ...
        private List<VistaClientes> BuscarCli()
        {
            try
            {
                if (txtCliente.Text == "" && !checkHabilitar.Checked && cmbFibra.SelectedIndex == -1) { LLenarListaCliente(true); }
                else
                {
                    var cliNegocio = new ClienteNegocio();
                    if (txtCliente.Text != "")
                    {
                        return cliNegocio.ObtenerListClientesPorNomyAp(txtCliente.Text);
                    }

                    if (checkHabilitar.Checked)
                    {
                        return cliNegocio.ObtenerListClientesPorEstado(cmbEstados.Text);
                    }
                    else
                    {
                        return cliNegocio.ObtenerListClientesPorTipo(cmbFibra.Text);
                    }
                }
                return null;
            }
        ...
        private void btnBuscar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(txtCliente.Text) && !checkHabilitar.Checked && cmbFibra.SelectedIndex == -1)
                {
                    LLenarListaCliente(true);
                }
            }
        ...
        private void SeleccionLv()
        {
            try
            {
                if (Seleccion)
                {
                    //if (listCliente.SelectedItems[0].SubItems[7].Text != "")
                    //{
                    //    MessageBox.Show("No se puede seleccionar un Cliente que fue dado de baja", "ERROR DE SELECCIÓN", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    //}
                    //else
                    //{
        ...
        private void listCliente_MouseClick(object sender, MouseEventArgs e)
        {
            //if (listCliente.SelectedItems.Count > 0)
            //{
            //    if (Users.Rol != TiposUsers.Administrador) return;
        ...
        private void frmFibraOptica_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Escape) Close();
